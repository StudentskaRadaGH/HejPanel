...
import { AsyncFunctionDetails } from "types";
...
type ServerActionWrapper = <A extends Function>(
    action: A,
) => (
    ...params: AsyncFunctionDetails<typeof action>["args"]
) => Promise<AsyncFunctionDetails<typeof action>["result"] | UserErrorType>;

export const serverAction: ServerActionWrapper =
    (action) =>
    async (...params) => {
        try {
            ...
            return UserError(error as string | Error | ZodError);
        }
    };
