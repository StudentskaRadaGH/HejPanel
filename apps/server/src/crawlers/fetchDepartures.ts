...
const fetchDeparturesFromURL = async (
    url: string,
    numberOfElements: number,
): Promise<Departure[]> => {
    try {
        const document = new JSDOM(await fetch(url).then((res) => res.text()))
            .window.document;
        ...
        if (document) {
            ...
            Array.from(
                document.querySelector(
                    "#col-content > div > div.box.connection > table > tbody",
                )?.children ?? [],
            ).forEach((row) => {
                if (numberOfElements <= 0) return;

                if (even) {
                    departures[departures.length - 1].carrier =
                        ((): Departure["carrier"] => {
                            switch (
                                row.children[1].querySelector("span")
                                    ?.textContent
                            ) {
                                ...
                                default:
                                    throw new Error(
                                        `Unknown carrier ${row.children[1].querySelector("span")?.textContent}`,
                                    );
                            }
                        })();
...
export default async function fetchDepartures(): Promise<Departures> {
    const departures: Departures = FAKE_DATA
        ...
        : {
              ladova: await fetchDeparturesFromURL(
                  "https://idos.cz/vlakyautobusymhdvse/odjezdy/vysledky/?f=Ladova&fc=305003",
                  3,
              ),
              natrati: await fetchDeparturesFromURL(
                  "https://idos.cz/vlakyautobusymhdvse/odjezdy/vysledky/?f=Na%20Trati&fc=305003",
                  3,
              ),
              vlak: await fetchDeparturesFromURL(
                  "https://idos.cz/vlakyautobusymhdvse/odjezdy/vysledky/?f=Olomouc-Hej%C4%8D%C3%ADn&fc=100003",
                  3,
              ),
          };
