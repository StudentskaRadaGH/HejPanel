...
const fetchCanteens = async (): Promise<
    { date: number; canteen: Canteen }[]
> => {
    const canteenResponse: Either<
        { state: "error" },
        { datum: string; nazev: string; druh: string }[][][]
    > = await fetch("https://app.strava.cz/api/jidelnicky", {
        method: "POST",
        headers: {
    ...
    if (
        canteenResponse.state === "error" ||
        typeof canteenResponse[0] === "undefined" ||
        canteenResponse[0].length === 0
    ) {
    ...
    const canteens: { date: number; canteen: Canteen }[] = [];
    ...
    for (const key in canteenResponse[0]) {
        ...
        if (
            lunchesProcessed.lunch1 &&
            lunchesProcessed.lunch2 &&
            lunchesProcessed.lunch3
        ) {
            const minLunchLength = Math.min(
                lunchesProcessed.lunch1.length,
                lunchesProcessed.lunch2.length,
                lunchesProcessed.lunch3.length,
            );

            for (let i = 0; i <= minLunchLength; i++) {
                if (
                    lunchesProcessed.lunch1[
                        lunchesProcessed.lunch1.length - i
                    ] ===
                        lunchesProcessed.lunch2[
                            lunchesProcessed.lunch2.length - i
                        ] &&
                    lunchesProcessed.lunch2[
                        lunchesProcessed.lunch2.length - i
                    ] ===
                        lunchesProcessed.lunch3[
                            lunchesProcessed.lunch3.length - i
                        ]
                ) {
                    commonSuffix.push(
                        lunchesProcessed.lunch1[
            ...
            for (let i = 0; i <= minLunchLength; i++) {
                if (
                    ...
                    lunchesProcessed.lunch2[lunchesProcessed.lunch2.length - i]
                ) {
                    commonSuffix.push(
                        lunchesProcessed.lunch1[
                            lunchesProcessed.lunch1.length - i
        ...
        if (commonSuffix.length > 0 && commonSuffix.join().trim() !== "") {
            ...
            if (day.lunch1)
                day.lunch1 = day.lunch1
                    .slice(0, -day.commonSuffix.length)
                    .trim()
                    .replace(/,$/, "");
            if (day.lunch2)
                day.lunch2 = day.lunch2
                    .slice(0, -day.commonSuffix.length)
                    .trim()
                    .replace(/,$/, "");
            if (day.lunch3)
                day.lunch3 = day.lunch3
                    .slice(0, -day.commonSuffix.length)
                    .trim()
                    .replace(/,$/, "");
        ...
        const date = canteenResponse[0][key][0].datum
            .split(".")
            .map((x) => parseInt(x));
        canteens.push({
            date: CanteenManager.getDateKey(
                new Date(date[2], date[1] - 1, date[0], 0, 0, 0, 0),
            ),
            canteen: day,
        });
    }
