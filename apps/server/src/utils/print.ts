...
export const printStartupScreen = () => {
	...
	const backendPort = privateEnv.SERVER_PORT;
	const backendPortLength = backendPort.toString().length;
	const wsPort = privateEnv.WS_PORT;
	const wsPortLength = wsPort.toString().length;
	const frontendUrl = privateEnv.CLIENT_URL;
	const adminUrl = privateEnv.ADMIN_URL;

	const maxLength = Math.max(backendPortLength, wsPortLength, frontendUrl.length) + 41;
	...
	console.log(white("│"), cyan("✅ API server running on port:      "), bgBrightBlue(black(` ${backendPort} `)) + emptySpace(backendPortLength), white("│"));
	console.log(white("│"), green("✅ WebSocket server running on port:"), bgBrightGreen(black(` ${wsPort} `)) + emptySpace(wsPortLength), white("│"));
	console.log(white(`├${horizontalLine}┤`));
